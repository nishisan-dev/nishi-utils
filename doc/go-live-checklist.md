...
| F5 | Crash abrupto: offsets recuperados (sem duplicatas) | `NGridDurabilityTest.shouldRecoverOffsetAfterCrashAndNotReceiveDuplicates` | ☐ (Docker) |
| F6 | Crash do seed: mensagens recuperadas | `NGridDurabilityTest.shouldRecoverMessagesAfterSeedCrash` | ☐ (Docker) |
| F7 | Multi-client crash: offsets independentes | `NGridDurabilityTest.shouldMaintainIndependentOffsetsAfterMultiClientCrash` | ☐ (Docker) |
| F8 | Restart total: zero duplicatas e zero perdas | `NGridTestcontainersSmokeTest` (2 cenários) | ☐ (Docker) |
...
| # | Critério | Evidência | Status |
|---|---|---|---|
| O1 | Runbook: Partição de rede | `docs/runbooks/network_partition.md` | ☑ (escrito, não validado em incidente) |
| O2 | Runbook: Líder instável | `docs/runbooks/unstable_leader.md` | ☑ (escrito, não validado em incidente) |
| O3 | Runbook: Recuperação pós-crash | `docs/runbooks/post_crash_recovery.md` | ☑ (escrito, não validado em incidente) |
| O4 | Runbook: Consistência pós-incidente | `docs/runbooks/post_incident_consistency.md` | ☑ (escrito, não validado em incidente) |
