...
## 0. Acabamentos do M1a (achados baixos do Refuter, faça primeiro)
- `protocol/ReadRequest`: teste de que o compact constructor rejeita `windowMs`, `targetStepSec`, `maxPoints` ≤ 0 e nulos; corrija o Javadoc que diz "os mesmos invariantes de `ViewQuery`" (a validação aqui é mais estrita: `ViewQuery` não rejeita janela zero).
- `placement/LeastLoadedPlacementPolicyTest`: acrescente permutação (6 ordens) com carga efetiva EMPATADA e capacidade mista → sempre o mesmo vencedor pelo `fillRatio`/`nodeId`.
- `protocol/Commands`: Javadoc em `LEADER_COMMANDS` registrando que `ADMIN_METRICS` fica de fora deliberadamente (é atendido por qualquer nó).
