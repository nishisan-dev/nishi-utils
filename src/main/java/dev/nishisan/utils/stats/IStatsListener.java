 ...
 * different statistics counters such as average counters, current value
 * counters,
 * and hit counters. Implementations can use these callbacks to perform specific
 * actions when the respective events occur.
 *
 * @param <E> the numeric element type for counter values
 */
public interface IStatsListener<E extends Number> {
    /**
     * Callback method triggered when an average counter is created.
     *
     * @param list the FixedSizeList instance representing the container for the
     *             counter's data.
     *             This list has a fixed capacity and ensures older elements are
     *             removed as new ones are added when at capacity.
     */
     ...
     *              containing information such as the counter's name and its
     *              initial value.
     */
    public void onCurrentValueCounterCreated(SimpleValueDTO value);

    /**
     * Callback method triggered when the value of a current value counter is
     * updated.
     *
     * @param value the SimpleValueDTO instance representing the updated counter,
     *              containing information such as the counter's name and its new
     *              value.
     */
     ...
     *               This object contains details such as the counter's name,
     *               current value,
     *               last calculation time, current rate, and the last update
     *               timestamp.
     */
     ...
     *               that has been incremented. This object contains details such as
     *               the counter's name,
     ...
     *
     * @param metric the HitCounterDTO instance representing the hit counter
     *               that was removed. This object contains information such as the
     *               counter's name,
     *               current value, last calculation time, current rate, last value,
